  ...
  sPreco:=  CurrToStr( cPreco);
   if ACBrETQ.Modelo in [etqPpla, etqPplb] then
    begin
      ...
      ACBrETQ.ImprimirTexto(orNormal, 2, 2, 2, 3, 3,'R$ / kg ' + Fpreco, 0, True);
    ...
    begin
      ACBrETQ.ImprimirTexto(orNormal, '0', 60, 60, 3, 3, lblDescricaoProduto.Caption, 0, True);
      ACBrETQ.ImprimirTexto(orNormal, 'A', 80, 80, 3, 3, '__________________________');
      ACBrETQ.ImprimirTexto(orNormal, 'A', 100, 100, 15, 50, 'Peso   ' +lblPeso.Caption);
      ACBrETQ.ImprimirTexto(orNormal, 'A', 120, 120, 3, 3,'R$ / kg ' + Fpreco, 0, True);
      ACBrETQ.ImprimirTexto(orNormal, 'A', 140, 140, 15, 50,sPreco );
      ACBrETQ.ImprimirTexto(orNormal, 'A', 160, 160, 15, 50,'D. EMB. '+ FormatDateTime('dd.mm.yy',now ));
      ACBrETQ.ImprimirTexto(orNormal, 'A', 180, 180, 15, 50,'D. VAL. '+ FormatDateTime('dd.mm.yy',IncDay(now,StrToInt(FValidade )) ));
      ACBrETQ.ImprimirTexto(orNormal, 'A', 200, 200, 15, 50,'data hora '+ FormatDateTime('hh:mm',now ));
      ACBrETQ.ImprimirBarras(orNormal, barEAN13, 220, 200, 13, 5,'234567', 10, becSIM);
    end;
   ACBrETQ.Desativar;
end;
...
begin
  FConfigImp := TConfigImpressoraINI
    (FFuncoes.SerializarINI('IMPRESSORA.INI', 'CONFIG', FConfigImp));
  ACBrETQ.Desativar;
